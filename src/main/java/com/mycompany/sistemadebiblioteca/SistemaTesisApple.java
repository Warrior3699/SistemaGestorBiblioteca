...
public class SistemaTesisApple {
   ...
   private void cargarTesis(String busqueda) {
    ...
    try (PreparedStatement stmt = conn.prepareStatement(sql)) {
        ...
        ResultSet rs = stmt.executeQuery();
        while (rs.next()) {
            // 1) Lee numero como String
            String numero = rs.getString("numero");
            // 2) Lee el resto igual
    ...
    private void mostrarFormularioTesis(int idTesis, String usuarioTesis) {
        ...
        card.add(crearEtiqueta("Número*"));      card.add(txtNumero);
        card.add(crearEtiqueta("Título*"));      card.add(txtTitulo);
        card.add(crearEtiqueta("Licenciatura*"));card.add(txtLicenciatura);
        card.add(crearEtiqueta("Autor*"));       card.add(txtAutor);
        card.add(crearEtiqueta("Asesor*"));      card.add(txtAsesor);
        card.add(crearEtiqueta("Estado"));       card.add(cmbEstado);
        card.add(crearEtiqueta("Mes"));          card.add(txtMes);
        card.add(crearEtiqueta("Año"));          card.add(txtAnio);
        ...
        tabla.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(
                ...
                if (col == 6) {
                    ...
                    Color color;
                    switch(est) {
                        case "Finalizada": color = new Color(52,199,89); break;
                        case "En revisión": color = new Color(255,149,0); break;
                        case "Publicada": color = new Color(88,86,214); break;
                        default: color = new Color(142,142,147);
                    }
